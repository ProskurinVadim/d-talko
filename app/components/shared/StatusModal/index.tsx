...
const StatusModal= ({ status }: Props) => {
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="27" height="27" viewBox="0 0 27 27" fill="none">
            ...
            <path d="M2.3335 2.3335L25.249 25.249" stroke="#D3D3D3" strokeWidth="3" strokeLinecap="round" />
                ...
                <div className="lg:max-w-[474px]">
                    <SectionTitle
                        tlwVar={{ color: status === 'error' ? 'danger' : 'main_dark', type: 'modal' }}
                        className="mt-6"
                    >
                        {data.title}
                    </SectionTitle>
                    <Text
                        tlwVar={{ type: 'normal', color: 'additional_light', leading: 'normal' }}
                        className="mt-4"
                    >
                        {data.text}
                    </Text>
                    <Link className={`block mt-12 ${button({ type: 'primary' })}`} href="/">
                        На головну
                    </Link>
                </div>
            </div>
